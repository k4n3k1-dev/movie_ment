...
export class TestLevel {
  ...
  _buildHouse() {
    ...
    this._window(-7.6, HOUSE.z0 + 0.02, 0, 3.6, 1.55);
    this._window(-6.8, HOUSE.z1 - 0.02, 0, 3.6, 1.65);
    this._window(6.2, HOUSE.z1 - 0.02, 0, 3.8, 1.65);
    this._window(HOUSE.x1 - 0.02, 2.0, Math.PI / 2, 3.0, 1.5);
  ...
  _buildYard() {
    ...
    for (const [x, z] of [[-15, -5], [15, -3], [-15, 9], [14, 11]]) {
      ...
      this.scene.add(bed);
      this._registerSolid([x - 2.1, 0, z - 0.7], [x + 2.1, 0.12, z + 0.7]);
      for (let i = 0; i < 5; i++) {
        ...
        this.scene.add(bush);
        const br = 0.5 + (i % 2) * 0.12;
        this._registerSolid([bush.position.x - br, 0, bush.position.z - br], [bush.position.x + br, 0.9, bush.position.z + br]);
      }
    }
  ...
  _buildFurniture() {
    // The furniture is laid out against the actual room footprints.  Each room
    // is deliberately given clear circulation space so objects do not overlap
    // walls, door openings, or one another.

    // LIVING ROOM: x=-12..2, z=-8..4
    this._sofa(-7.1, -6.65, 0);
    this._coffeeTable(-7.1, -4.55);
    this._rug(-7.1, -4.75, 4.8, 3.0);
    this._tvUnit(-7.1, 3.45);
    this._plant(-10.3, -0.8, 0.9);

    // BEDROOM 1: x=-12..-2, z=4..10
    this._bed(-7.0, 7.15, 0);
    this._bedside(-4.95, 7.15);
    this._dresser(-10.65, 8.65, 0);
    this._crawlUnderFurniture(-7.0, 7.15, 2.55, 4.0, 1.02);

    // KITCHEN: x=2..12, z=0..5
    this._kitchenCabinet(5.45, 4.42);
    this._counter(10.85, 3.05, 1.25);
    this._fridge(10.95, 1.15);
    this._diningTable(5.35, 2.0);
    for (const [x, z] of [[3.65, 1.0], [7.05, 1.0], [3.65, 3.0], [7.05, 3.0]]) this._chair(x, z);

    // BEDROOM 2: x=2..12, z=5..10
    this._bed(6.55, 7.45, 0);
    this._bench(9.15, 5.55, 2.1, 0.58);
    this._wardrobe(10.95, 8.35);
    this._bedside(3.95, 7.45);
    this._crawlUnderFurniture(6.55, 7.45, 2.55, 4.0, 1.02);

    // BATHROOM: x=7..12, z=-8..0
    this._vanity(8.25, -6.65);
    this._toilet(10.8, -6.15);
    this._bathtub(9.35, -2.05);

    // Low pieces in the circulation areas are climbable but kept clear of doors.
    this._bench(-1.0, 1.35, 1.8, 0.58);
    this._bench(0.15, -4.8, 1.5, 0.55);
  }
  ...
  _exteriorDoor(mat, x, z) {
    const width = DOOR_W;
    const hingeX = x - width / 2;
    const pivot = new THREE.Group();
    pivot.position.set(hingeX, 0, z);
    this.scene.add(pivot);
    const slab = new THREE.Mesh(new THREE.BoxGeometry(width, 2.55, 0.12), mat);
    slab.position.set(width / 2, 1.275, 0); slab.castShadow = true; pivot.add(slab);
    const knob = new THREE.Mesh(new THREE.SphereGeometry(0.055, 12, 8), this._material(0xb98b3f, 0.25, 0.8));
    knob.position.set(width - 0.16, 1.2, -0.09); pivot.add(knob);
  ...
  _interiorDoor(mat, x, z, rotation) {
    const width = 1.48;
    const pivot = new THREE.Group();
    // For a vertical wall the door runs along Z, for a horizontal wall along X.
    // The pivot is placed at the left/bottom jamb and the slab is offset by half width.
    if (rotation === 0) pivot.position.set(x, 0, z - width / 2);
    else pivot.position.set(x - width / 2, 0, z);
    pivot.rotation.y = rotation;
    this.scene.add(pivot);
    const slab = new THREE.Mesh(new THREE.BoxGeometry(0.12, 2.45, width), mat);
    slab.position.set(0, 1.225, width / 2); slab.castShadow = true; pivot.add(slab);
    const knob = new THREE.Mesh(new THREE.SphereGeometry(0.05, 10, 8), this._material(0xb98b3f, 0.25, 0.8));
    knob.position.set(0.1, 1.18, width - 0.16); pivot.add(knob);
    this.doors.push({ pivot, isOpen: false, currentAngle: 0, targetAngle: 0, openAngle: Math.PI / 2 });
  }
  ...
  _registerSolid(min, max) {
    this.solidColliders.push(new THREE.Box3(new THREE.Vector3(...min), new THREE.Vector3(...max)));
  }

  _box(mesh, x, y, z, sx, sy, sz, climbable = false, solid = !climbable) {
    mesh.position.set(x, y + sy / 2, z);
    mesh.scale.set(sx, sy, sz);
    mesh.castShadow = true; mesh.receiveShadow = true;
    this.scene.add(mesh);
    if (climbable) this._registerPlatform(mesh, [x - sx / 2, y, z - sz / 2], [x + sx / 2, y + sy, z + sz / 2]);
    else if (solid) this._registerSolid([x - sx / 2, y, z - sz / 2], [x + sx / 2, y + sy, z + sz / 2]);
    return mesh;
  }
  ...
  _sofa(x, z, rot) {
    ...
    for (const px of [-1.48, 1.48]) { const arm = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.9, 1.1), fabric); arm.position.set(px, 0.82, 0); arm.castShadow = true; g.add(arm); this._registerSolid([x + px - 0.11, 0.37, z - 0.55], [x + px + 0.11, 1.27, z + 0.55]); }
    this._registerSolid([x - 1.65, 0.94, z - 0.58], [x + 1.65, 1.58, z - 0.36]);
  ...
  _bed(x, z, rot) {
    ...
    const blanket = new THREE.Mesh(new THREE.BoxGeometry(2.52, 0.08, 2.25), this._material(0x6b7f86, 0.93));
    blanket.position.set(0, 0.86, 0.55); blanket.castShadow = true; g.add(blanket);
    for (const px of [-0.67, 0.67]) {
      const pillow = new THREE.Mesh(new THREE.BoxGeometry(1.05, 0.16, 0.62), this._material(0xf1ede4, 0.96));
      pillow.position.set(px, 0.88, -1.45); pillow.rotation.x = -0.04; pillow.castShadow = true; g.add(pillow);
    }
    ...
    worldBox.min.y = 0.83; worldBox.max.y = 1.04; worldBox.userData = { isClimbable: true };
    this.platforms.push({ mesh: mattress, box: worldBox });
  }
  ...
  _dresser(x, z, rot) {
    ...
    this._registerSolid([x - 0.95, 0, z - 0.28], [x + 0.95, 1.15, z + 0.28]);
  }
  ...
  _tree(x, z, scale) {
    ...
    const trunkR = 0.52 * scale;
    this._registerSolid([x - trunkR, 0, z - trunkR], [x + trunkR, 4.6 * scale, z + trunkR]);
  ...
  _fencePost(mat, x, z, rotation) {
    ...
    this._registerSolid([x - 0.12, 0, z - 0.12], [x + 0.12, 1.2, z + 0.12]);
    const rail = new THREE.Mesh(new THREE.BoxGeometry(2.5, 0.12, 0.12), mat); rail.position.set(x, 0.9, z); rail.rotation.y = rotation; rail.castShadow = true; this.scene.add(rail);
    if (!rotation) this._registerSolid([x - 1.25, 0.84, z - 0.08], [x + 1.25, 0.96, z + 0.08]);
    else this._registerSolid([x - 0.08, 0.84, z - 1.25], [x + 0.08, 0.96, z + 1.25]);
  }

  getColliders() {
    return [...this.wallColliders, ...this.solidColliders, ...this.platforms.map(p => p.box)];
  }
