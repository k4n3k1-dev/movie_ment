...
export class TestLevel {
  constructor(scene) {
    ...
    for (const [xMin, xMax] of roomBounds) {
      const light = new THREE.PointLight(0xffe8c2, 1.2, 30);
      light.position.set((xMin + xMax) / 2, 2.6, 0);
      light.castShadow = true;
      scene.add(light);
      this.rooms.push({ xMin, xMax, light, baseIntensity: 0.9, litIntensity: 1.5 });
    }
  }
  ...
  update(delta, playerPos = null) {
    ...
    if (playerPos) {
      for (const room of this.rooms) {
        // Keep all rooms evenly lit in the daytime; the lights are only a
        // subtle fill under the daylight key light.
        room.light.intensity += (room.litIntensity - room.light.intensity) *
          Math.min(1, 2 * delta);
      }
    }
  }
