...
export class PlayerController {
  ...
  _bind() {
    window.addEventListener('keydown', (e) => {
      ...
      const wantsStand = this.crawling;
      if (wantsStand && this.level?.isBlockedByLowCeiling(this.player.getPosition(), this.collisionRadius, this.standingHeight)) return;
      this.crawling = !this.crawling;
    }
    });
  ...
  update(delta) {
    ...
    if (moving) {
      ...
      const candidate = playerObj.position.clone().add(move);
      // Low furniture (beds/benches) becomes a genuine crawl passage: standing
      // characters cannot walk into its clearance volume, but crawling characters can.
      const blockedLow = !this.crawling && this.level?.isBlockedByLowCeiling(candidate, this.collisionRadius, this.standingHeight);
      if (!blockedLow) {
        playerObj.position.copy(candidate);
