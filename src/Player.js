...
export class Player {
  constructor() {
    ...
    const fabricMat = new THREE.MeshStandardMaterial({ color: 0x25282d, roughness: 0.9 });
    ...
    const jacketHem = new THREE.Mesh(new THREE.CylinderGeometry(0.30, 0.34, 0.18, 16), jacketMat);
    jacketHem.scale.z = 0.72;
    jacketHem.position.y = 0.89;
    jacketHem.castShadow = true;
    this.root.add(jacketHem);
    for (const x of [-0.105, 0.105]) {
      const chestPanel = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.48, 0.015), fabricMat);
      chestPanel.position.set(x, 1.18, 0.19);
      this.root.add(chestPanel);
    }
    ...
    const neck = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.14, 0.20, 16), maskMat);
    neck.position.y = 1.49;
    neck.castShadow = true;
    this.root.add(neck);
    const hood = new THREE.Mesh(new THREE.TorusGeometry(0.22, 0.055, 8, 20, Math.PI * 1.55), jacketMat);
    hood.rotation.x = Math.PI / 2;
    hood.rotation.z = Math.PI;
    hood.position.set(0, 1.48, -0.035);
    hood.scale.z = 0.8;
    ...
    const pack = new THREE.Mesh(new THREE.BoxGeometry(0.38, 0.62, 0.16), pantsMat);
    pack.position.set(0, 1.20, -0.20);
    pack.castShadow = true;
    this.root.add(pack);
    for (const x of [-0.16, 0.16]) {
      const strap = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.55, 0.035), fabricMat);
      strap.position.set(x, 1.23, 0.17);
  ...
  _buildLeg(pantsMat, bootMat, soleMat) {
    ...
    hip.add(knee);
    const kneePad = new THREE.Mesh(new THREE.SphereGeometry(0.105, 12, 8), bootMat);
    kneePad.scale.set(0.9, 0.55, 0.55);
    kneePad.position.set(0, 0.01, 0.075);
    ...
    knee.add(foot);
    const cuff = new THREE.Mesh(new THREE.CylinderGeometry(0.085, 0.09, 0.09, 12), bootMat);
